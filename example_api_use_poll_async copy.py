...
from pydabpumps.data import DabPumpsLogin, DabPumpsStatusCode
from tests import TEST_USERNAME, TEST_PASSWORD


async def main():
    api = None
    test_method= None #[DabPumpsLogin.ACCESS_TOKEN, DabPumpsLogin.REFRESH_TOKEN, DabPumpsLogin.DCONNECT_APP]

    # Process these calls in the right order
    api = AsyncDabPumps(TEST_USERNAME, TEST_PASSWORD)
    await api.login(test_method=test_method)
    ...
    for t in range(60):
        try:
            ...
            count_success += 1
            logger.debug(f"count success: {count_success}, fail: {count_fail}")
