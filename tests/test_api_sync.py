...
class TestContext:
    def __init__(self):
        self.api = None
        self.framework = TEST_FRAMEWORK_SYNC
...
def test_push_data(name, method, loop, exp_except, request):
    ...
    def device_state_handler(serial: str, state: DabPumpsDeviceState):
    ...
    if context.framework != 'async':
        ...
        for device_serial in context.api.device_map.keys():
            with pytest.raises(NotImplementedError):
                context.api.on_device_state(device_serial, device_state_handler)
...
def test_set_data_by_value(method, key, lang, exp_value, exp_except, request):
    context = request.getfixturevalue("context")
    context.api = DabPumps(TEST_USERNAME, TEST_PASSWORD)
    assert context.api.closed == False
...
def test_decode(name, serial, key, code, exp_value, request):
    context = request.getfixturevalue("context")
    context.api = DabPumps("dummy_usr", "wrong_pwd") # no login needed
...
def test_metadata(name, serial, key, exp_type, exp_values, exp_unit, request):
    context = request.getfixturevalue("context")
    context.api = DabPumps("dummy_usr", "wrong_pwd") # no login needed
